    ...
    <section class="container mt-4 mb-10 mx-auto shadow-sm text-xs">
        <table class="table-auto w-full">
            ...
            <tbody class="divide-y divide-gray-200">
                <tr v-for="user in users.data" v-bind="user.id" class="hover:bg-slate-200">
                    <td>{{ user.id }}</td>
                    <td>{{ user.name }}</td>
                    <td>{{ user.email }}</td>
                    <td>{{ user.cpf }}</td>
                    ...
                    <td class="flex justify-center space-x-1">
                        ...
                        <Link v-bind:href="(route('users-edit', user.id))">
                        ...
                        <button @click="destroy(user.id)" class="p-1 text-white rounded bg-rose-600/80 hover:bg-red-300">
                            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" fill="currentColor" class="w-4 h-4" viewBox="0 0 26 26">
